...
source $ROOT_PATH/default_vars.sh
alias help=$'cat $ROOT_PATH/default_vars.sh  |sed \'s/=${/ /g\' |sed \'s/:=/ /g\' |sed \'s/}//g\' |sed \'s/ "/ - Default value: "/g\''
...
case $1 in
	...
	make)
		...
		if [ $TABLES = "full" ]; then
			...
			TYPE=$TABLES
			TABLES="--tables "$(mysql -u $2 -p$3 -h $MYSQL_HOST -D $4 -Bse \
	...
	"show tables where Tables_in_tgui like 'api\_%'" 2>/dev/null | xargs)"
		...
		umask 111; mysqldump -u $2 -p$3 -h $MYSQL_HOST $4 $TABLES | grep -v "Using a password" > $ROOT_PATH/backups/database/$(date '+%Y-%m-%d')_${END_OF_NAME}_${REVISION}.sql 2>&1 | grep -v "Using a password"
	...
	restore)
		...
		mysql -u $2 -p$3 -h $MYSQL_HOST $4 < $ROOT_PATH/backups/database/$5
